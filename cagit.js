...
//    console.log(data1)//max min avg values are there
    ...
    .append("line")
      ...
      .attr("fill", function(d){return getSubfeatureName(d.key)})
      .style("width", 10)
      .style("opacity", 0.5)
    ...
    .enter()
    .append("line")
      .attr("x1", function(d){return(getMaxpos(d.key,d.value.max))})
      .attr("x2", function(d){return(getMaxpos(d.key,d.value.max))})
      .attr("y1", function(d){return(y(d.key)+y.bandwidth())})
      .attr("y2", function(d){return(y(d.key) )})
   
      .attr("stroke", function(d){return getSubfeatureName(d.key)})
      .attr("fill", function(d){return getSubfeatureName(d.key)})
...
//eğimli köşegen
        ...
        .append("line")
            // .attr("x1", function(d){return(x(d.value.max))})
            .attr("x1", function(d){return(getLastLines(d.key,d.value.max))})
            .attr("x2", function(d){return(x(d.value.min+d.value.avg/3))})
            .attr("y1", function(d){return(y(d.key)+y.bandwidth()/2)})
            .attr("y2", function(d){return(y(d.key) )})
            .attr("stroke", function(d){return getSubfeatureName(d.key)})
            .style("width", 10)
        ...
        svg
            ...
            .enter()
            .append("rect")
                .attr("x", function(d){return(x(d.value.min+d.value.avg/5))}) // console.log(x(d.value.q1)) ;
                ...
                .attr("width", function(d){return(getValues(d.key,d.value.avg))})
                ...
                .attr("fill", function(d){console.log(d.key);return getSubfeatureName(d.key)})
                .attr("stroke", function(d){return getSubfeatureName(d.key)})
                .style("opacity", 0.6)
...
//   }
    ...
    svg
      ...
      .enter()
      .append("circle")
        .attr("cx", function(d){return(x(parseInt(d.value)))})
        ...
        .on("mousemove", function(d){return (tooltip2.html("Class: "+transform(givenclass)+"<br>Feature: "+givenTopic+"<br>Feature: "+d.path[0]["__data__"].key+"<br> Value: "+Math.floor(d.path[0]["__data__"].value))).style("top", (event.pageY-150)+"px").style("left",(event.pageX-150)+"px");})
